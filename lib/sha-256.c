...
void *memcpy(void *dest, const void *src, u8 n) {
    char *dp = dest;
    const char *sp = src;
    while (n--)
        *dp++ = *sp++;
...
static void init_buf_state(struct buffer_state *state, const void *input, u8 len) {
    state->p = input;
    state->len = len;
    state->total_len = len;
...
static int calc_chunk(uint8_t chunk[CHUNK_SIZE], struct buffer_state *state) {
    ...
    if (space_in_chunk >= TOTAL_LEN_LEN) {
        const u8 left = space_in_chunk - TOTAL_LEN_LEN;
        u8 len = state->total_len;
        int i;
...
const uint32_t initial_hash_value[] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};
        ...
        for (i = 0; i < 4; i++) {
            ...
            for (j = 0; j < 16; j++) {
                if (i == 0) {
                    ...
                    const uint32_t s1 = right_rot(w[(j + 14) & 0xf], 17) ^right_rot(w[(j + 14) & 0xf], 19) ^
                                        (w[(j + 14) & 0xf] >> 10);
